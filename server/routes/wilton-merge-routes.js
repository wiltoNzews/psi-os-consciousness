...
router.get('/merge/status', (req, res) => {
  try {
    ...
    // Usar arquivo de resposta JSON já preparado
    const responseJsonPath = path.join(__dirname, 'wilton-merge-response.json');
    
    if (fs.existsSync(responseJsonPath)) {
      // Ler o arquivo JSON de resposta
      const jsonContent = fs.readFileSync(responseJsonPath, 'utf8');
      const responseData = JSON.parse(jsonContent);
      
      res.json(responseData);
    } else {
      // Se o arquivo não existir, usar o método original
...
router.get('/merge/progress', (req, res) => {
  try {
    // Usar arquivo de resposta JSON já preparado para progresso
    const progressJsonPath = path.join(__dirname, 'wilton-merge-progress.json');
    
    if (fs.existsSync(progressJsonPath)) {
      // Ler o arquivo JSON de resposta de progresso
      const jsonContent = fs.readFileSync(progressJsonPath, 'utf8');
      const progressData = JSON.parse(jsonContent);
 ...
 */
async function processMergeInBackground(options) {
  try {
    ensureDirectories();
    ...
    try {
      await execAsync(`unzip -o "${CHATGPT_EXPORT_ZIP}" -d "${CHATGPT_EXPORT_DIR}"`);
      appendToLog(logPath, `[SUCCESS] Arquivo ZIP extraído com sucesso`);
    } catch (error) {
      appendToLog(logPath, `[ERROR] Falha ao extrair arquivo ZIP: ${error.message}`);
      return;
    }
    ...
    let concepts = new Set(['surge mode', 'coherence oscillation', 'reincarnation', 'qctf']);
    let tags = new Set(['#GPT', '#SELF', '#SYSTEM', '#GLIFO']);
    ...
    for (let i = 0; i < batches; i++) {
      ...
      for (const file of batch) {
        try {
          ...
          appendToLog(logPath, `[WARNING] Erro ao processar arquivo ${file}: ${error.message}`);
        }
      }
    ...
    if (fs.existsSync(memoryKeysPath)) {
      try {
        ...
        appendToLog(logPath, `[ERROR] Erro ao atualizar metadados: ${error.message}`);
      }
    }
    ...
    const logPath = path.join(WILTONOS_CORE_DIR, 'Logs', 'processing.log');
    appendToLog(logPath, `[ERROR] Erro no processamento em background: ${error.message}`);
  }
}
