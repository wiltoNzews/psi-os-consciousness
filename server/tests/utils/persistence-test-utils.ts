...
import { IPersistenceLayer } from '../../services/persistence-layer.js';

/**
 * Interface for MetaCognitiveEvent used in tests
 */
 ...
 */
export function createTestEvent(options: Partial<MetaCognitiveEvent> = {}): MetaCognitiveEvent {
  return {
 ...
 */
export function createComplexTestObject() {
  const now = new Date();
  return {
    id: `complex-${Date.now()}`,
    createdAt: now,
 ...
 */
export function createCircularTestObject() {
  const obj: any = {
 ...
 */
export async function verifyPersistenceImplementation(
