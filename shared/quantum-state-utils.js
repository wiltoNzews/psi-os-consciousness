 ...
 */
export function isAntiFlowState(state) {
 ...
 */
export function isPartialFlowState(state) {
 ...
 */
export function extractContextTag(state) {
 ...
 */
export function formatWithQuantumState(message, state) {
    ...
    return "[QUANTUM_STATE: ".concat(state, "] ").concat(message);
}
/**
 * Convert from the legacy separation of REALITY_MODE boolean and FlowType
 * to the unified QuantumState enum
 */
export function legacyToQuantumState(isRealityMode, flowType) {
    if (isRealityMode) {
        switch (flowType) {
            case 'FLOW':
 ...
 */
export function getDefaultQuantumState(nodeEnv) {
    if (nodeEnv === void 0) { nodeEnv = process.env.NODE_ENV || 'development'; }
