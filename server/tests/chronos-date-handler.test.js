...
function testSerializationDeserialization() {
  ...
  // With our implementation, serialized date is an object with SERIALIZED_DATE_KEY
  if (serialized && typeof serialized === 'object' && SERIALIZED_DATE_KEY in serialized) {
...
function testReplacerReviver() {
  ...
  // Verify the replaced format - just check if it has the SERIALIZED_DATE_KEY property
  if (replaced && replaced[SERIALIZED_DATE_KEY]) {
    ...
    if (revived instanceof Date && testDate.getTime() === revived.getTime()) {
      console.log('PASS: dateReplacer and dateReviver work correctly');
    } else {
      console.error('FAIL: Revived date does not match original');
      console.error('Original timestamp:', testDate.getTime());
      console.error('Revived timestamp:', revived instanceof Date ? revived.getTime() : 'Not a Date');
    }
  } else {
...
function testNestedObjectsWithDates() {
  ...
  // We know this test is failing - our implementation may need further work to handle deeply nested date objects correctly
  // For now, let's just check if the dates are being recognized at all and print the status
  
  let allDatesRestored = true;
  let allDatesMatch = true;
  
  // Check if dates are restored
  if (!(parsedObject.createdAt instanceof Date)) {
    allDatesRestored = false;
    console.error('FAIL: createdAt is not a Date instance');
  }
  
  if (!(parsedObject.metadata && parsedObject.metadata.lastChecked instanceof Date)) {
    allDatesRestored = false;
    console.error('FAIL: metadata.lastChecked is not a Date instance');
  }
  
  if (!(parsedObject.metadata && parsedObject.metadata.history && 
       parsedObject.metadata.history.firstSeen instanceof Date)) {
    allDatesRestored = false;
    console.error('FAIL: metadata.history.firstSeen is not a Date instance');
  }
  
  if (!(parsedObject.metadata && parsedObject.metadata.history && 
       parsedObject.metadata.history.lastModified instanceof Date)) {
    allDatesRestored = false;
    console.error('FAIL: metadata.history.lastModified is not a Date instance');
  }
  
  // Check date values only if they are Date instances
  if (parsedObject.createdAt instanceof Date && 
      testObject.createdAt.getTime() !== parsedObject.createdAt.getTime()) {
    allDatesMatch = false;
    console.error('FAIL: createdAt values do not match');
  }
  
  if (parsedObject.metadata && parsedObject.metadata.lastChecked instanceof Date && 
      testObject.metadata.lastChecked.getTime() !== parsedObject.metadata.lastChecked.getTime()) {
    allDatesMatch = false;
    console.error('FAIL: metadata.lastChecked values do not match');
  }
  ...
  if (parsedObject.metadata && parsedObject.metadata.history && 
    ...
    allDatesMatch = false;
    console.error('FAIL: metadata.history.firstSeen values do not match');
  }
  ...
  if (parsedObject.metadata && parsedObject.metadata.history && 
    ...
    allDatesMatch = false;
    console.error('FAIL: metadata.history.lastModified values do not match');
  }
  
  if (!allDatesRestored) {
    console.error('FAIL: Not all nested dates were properly restored');
  } else if (!allDatesMatch) {
...
function testArraysWithDates() {
  ...
  // Similar to nested objects, we need more detailed diagnostics for arrays
  let allDatesRestored = true;
  let allDatesMatch = true;
  
  if (!Array.isArray(parsedArray)) {
    console.error('FAIL: Parsed result is not an array');
    return;
  }
  
  // Verify each array element individually with detailed diagnostics
  for (let i = 0; i < testArray.length; i++) {
    if (i >= parsedArray.length) {
      console.error(`FAIL: Missing array element at index ${i}`);
      allDatesRestored = false;
      continue;
    }
    
    // Check if the parsed object has the expected structure
    if (!parsedArray[i] || typeof parsedArray[i] !== 'object') {
      console.error(`FAIL: Element at index ${i} is not an object`);
      allDatesRestored = false;
      continue;
    }
    
    // Check if the date property exists and is a Date instance
    ...
    // Check if the date matches the original
    if (testArray[i].date.getTime() !== parsedArray[i].date.getTime()) {
      ...
      console.error(`  Original: ${testArray[i].date.toISOString()}`);
      console.error(`  Parsed: ${parsedArray[i].date.toISOString()}`);
      allDatesMatch = false;
      continue;
    }
    
    console.log(`PASS: Array element ${i} correctly processed`);
  }
  
  if (allDatesRestored && allDatesMatch) {
    console.log('PASS: Arrays with dates are correctly processed');
  } else {
    console.error('FAIL: Not all array elements were correctly processed');
  }
}
