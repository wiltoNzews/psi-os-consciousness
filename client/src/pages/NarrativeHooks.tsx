...
const NarrativeHooksPage = () => {
  ...
  // Dados de exemplo para demonstração (para garantir que a interface funcione enquanto integramos com o backend)
  const mockStimuli: Stimulus[] = React.useMemo(() => [
    {
      id: 'yt:9LDCUvy0-Y8',
      type: 'youtube',
  ...
  // Separar estímulos calming e roughening baseado no efeito
  const calmingStimuli = React.useMemo(() => 
    mockStimuli.filter(s => s.effect < 0)
  ...
  const rougheningStimuli = React.useMemo(() => 
    mockStimuli.filter(s => s.effect > 0)
  ...
  // Dados de exemplo para pesos de feedback
  ...
  // Dados de exemplo para histórico de coerência
  const coherenceHistory = React.useMemo(() => {
  ...
  // Calculate current phi and target
  const currentPhi = coherenceHistory.length > 0 ? coherenceHistory[coherenceHistory.length - 1].phi : 0;
  const targetPhi = 0.75; // 3:1 ratio target
