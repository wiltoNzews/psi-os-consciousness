...
/**
 * 4W+1H component of the framework (contextual elements)
 */
export interface BaseContext {
  who: string[]; // Actors/stakeholders involved
  what: string; // Action or decision being analyzed
  when: {
    timeframe: string;
    urgency: number; // 1-10 scale (higher = more urgent)
    deadline?: Date;
  };
  where: {
    domain: string;
    location?: string;
    scope: 'local' | 'regional' | 'global' | 'system-wide' | string;
  };
  how: {
    methods: string[];
    resources: string[];
...
/**
 * WHICH component - strategic lens and priority aspects
 */
export interface StrategicLens {
  lens: 'efficiency' | 'innovation' | 'stability' | 'growth' | 'resilience' | string;
  aspects: Array<{
    name: string;
    priority: number; // 1-10 scale (higher = higher priority)
    tradeoffs: string[];
  }>;
...
/**
 * MACRO-OFFSET component - long-term systemic implications
 */
export interface MacroOffset {
  systemicImplications: Array<{
    area: string;
    timeline: 'short-term' | 'medium-term' | 'long-term';
    impact: number; // -10 to 10 scale (negative = negative impact)
    certainty: number; // 0-1 scale (higher = more certain)
  }>;
  offsetFactor: number; // Overall adjustment factor for decision
  adaptiveThreshold: number; // When to reconsider decision based on new data
}

/**
 * FLOW/ANTI-FLOW component - balance between momentum and disruption
 */
export interface FlowDynamics {
  currentMomentum: number; // 0-10 scale of current direction strength
  flowDirection: string; // Description of current flow direction
  disruptionValue: number; // 0-10 scale of value of disrupting current flow
  flowRatio: number; // Flow/Anti-Flow ratio (higher = more flow)
  equilibriumState: 'flow-dominant' | 'anti-flow-dominant' | 'balanced';
}

/**
 * Complete cognitive framework result
 */
export interface CognitiveFrameworkResult {
  id: string;
  timestamp: Date;
  context: BaseContext;
  ...
  decisionValue: number; // -100 to 100 scale (higher = better decision)
  confidenceScore: number; // 0-1 scale (higher = more confident)
  alternativeDecisions: Array<{
    description: string;
    relativeValue: number; // -100 to 100 compared to main decision
  }>;
  actionSteps: string[];
  warnings: string[];
 ...
 */
export function applyCognitiveExecutionFramework(
  context: BaseContext,
  strategicLens: StrategicLens,
  macroOffset: MacroOffset,
  flowDynamics: FlowDynamics
): CognitiveFrameworkResult {
  // 1. Validate context completeness
  const contextScore = evaluateContextCompleteness(context);
  
  // 2. Evaluate strategic lens alignment
  const lensAlignmentScore = evaluateStrategicLensAlignment(context, strategicLens);
  
  // 3. Calculate macro offset adjustment
  const macroAdjustment = calculateMacroOffsetAdjustment(macroOffset);
  
  // 4. Determine flow/anti-flow balance value
  const flowBalanceValue = calculateFlowBalance(flowDynamics);
  
  // 5. Calculate overall decision value 
  // Core formula: Decision(t) = (4W+1H)×WHICH(X,Y)×MACRO-OFFSET×(FLOW/ANTI-FLOW)
  const decisionValue = contextScore * lensAlignmentScore * macroAdjustment * flowBalanceValue;
  
  // Scale to -100 to 100 range
  const scaledDecisionValue = Math.max(-100, Math.min(100, decisionValue * 100));
  ...
  const alternativeDecisions = generateAlternativeDecisions(
    context,
    strategicLens,
    macroOffset,
    flowDynamics,
 ...
 */
function evaluateContextCompleteness(context: BaseContext): number {
  let score = 0;
  let maxScore = 0;
 ...
 * @returns Score between 0-1
 */
function evaluateStrategicLensAlignment(
  context: BaseContext,
  strategicLens: StrategicLens
): number {
  ...
  const aspects = strategicLens.aspects || [];
  if (aspects.length > 0) {
    ...
    const highPriorityAspects = aspects.filter(a => a.priority >= 7);
    for (const aspect of highPriorityAspects) {
      // Check if aspect is mentioned in context
      if (context.what.toLowerCase().includes(aspect.name.toLowerCase()) || 
          context.how.methods.some(m => m.toLowerCase().includes(aspect.name.toLowerCase()))) {
        alignment += 0.05;
      }
    }
  }
 ...
 */
function calculateMacroOffsetAdjustment(macroOffset: MacroOffset): number {
  ...
  if (macroOffset.systemicImplications && macroOffset.systemicImplications.length > 0) {
    ...
    for (const implication of macroOffset.systemicImplications) {
      // Convert timeline to weight (long-term implications get higher weight)
      let timelineWeight = 1.0;
      if (implication.timeline === 'medium-term') timelineWeight = 1.5;
      if (implication.timeline === 'long-term') timelineWeight = 2.0;
      
      // Weight by certainty and timeline
      const weight = implication.certainty * timelineWeight;
 ...
 */
function calculateFlowBalance(flowDynamics: FlowDynamics): number {
  ...
  const balance = Math.log(flowDynamics.flowRatio) / Math.log(3) + 1;
...
function calculateConfidenceScore(
  ...
  const contextScore = evaluateContextCompleteness(context) * 0.4;
  
  // 2. Strategic lens alignment contribution (30%)
  const lensScore = evaluateStrategicLensAlignment(context, strategicLens) * 0.3;
  
  // 3. Macro offset certainty contribution (15%)
  let offsetCertainty = 0.5; // Default moderate certainty
  
  if (macroOffset.systemicImplications && macroOffset.systemicImplications.length > 0) {
    // Average certainty across all implications
    const avgCertainty = macroOffset.systemicImplications.reduce(
      (sum, imp) => sum + imp.certainty, 
      0
    ) / macroOffset.systemicImplications.length;
    
    offsetCertainty = avgCertainty;
  }
  
  const offsetScore = offsetCertainty * 0.15;
  
  // 4. Flow dynamics clarity contribution (15%)
  let flowClarity = 0.5; // Default moderate clarity
  ...
  const flowScore = flowClarity * 0.15;
 ...
 */
function generateAlternativeDecisions(
  context: BaseContext,
  strategicLens: StrategicLens,
  macroOffset: MacroOffset,
  flowDynamics: FlowDynamics,
  mainDecisionValue: number
): Array<{
  description: string;
  ...
  if (strategicLens.alternativeLenses && strategicLens.alternativeLenses.length > 0) {
    const altLens = strategicLens.alternativeLenses[0];
    
    alternatives.push({
      description: `Apply ${altLens} lens instead of ${strategicLens.lens}`,
  ...
  if (flowDynamics.flowRatio !== undefined) {
    const inverseDynamics = flowDynamics.flowRatio > 1
  ...
  const timeframeShift = context.when.timeframe.toLowerCase().includes('long')
  ...
  const scopeShift = context.where.scope === 'local' || context.where.scope === 'regional'
  ...
  return alternatives.sort((a, b) => b.relativeValue - a.relativeValue);
}
...
function generateActionSteps(
  ...
  steps.push(`Define clear metrics for "${context.what}" success aligned with ${strategicLens.lens} lens`);
  
  // Add stakeholder-focused actions
  if (context.who && context.who.length > 0) {
    steps.push(`Engage key stakeholders: ${context.who.join(', ')}`);
  }
  
  // Add method-focused actions
  if (context.how && context.how.methods && context.how.methods.length > 0) {
    // Pick primary method
    const primaryMethod = context.how.methods[0];
    steps.push(`Implement using ${primaryMethod} as primary approach`);
    
    // If multiple methods, add integration step
    if (context.how.methods.length > 1) {
      steps.push(`Create integration plan for multiple methods: ${context.how.methods.join(', ')}`);
    }
  }
  
  // Add resource allocation action
  if (context.how && context.how.resources && context.how.resources.length > 0) {
    steps.push(`Allocate resources: ${context.how.resources.join(', ')}`);
  }
  
  // Add timeline-based action
  if (context.when) {
    if (context.when.deadline) {
      steps.push(`Set milestone timeline to meet deadline: ${context.when.deadline.toDateString()}`);
    } else {
      steps.push(`Establish timeline appropriate for ${context.when.timeframe} timeframe`);
    }
  }
  
  // Add high-priority aspect focus
  if (strategicLens.aspects && strategicLens.aspects.length > 0) {
    const highPriorityAspects = strategicLens.aspects
      .filter(a => a.priority >= 8)
      .map(a => a.name);
      
    if (highPriorityAspects.length > 0) {
      steps.push(`Prioritize these critical aspects: ${highPriorityAspects.join(', ')}`);
    }
  }
  
  // Add confidence-based actions
  if (confidenceScore < 0.7) {
    steps.push(`Implement validation steps to address uncertainty (confidence: ${(confidenceScore * 100).toFixed(0)}%)`);
  }
  
  // Add value-based scope suggestion
  if (Math.abs(decisionValue) > 70) {
    steps.push(`Expand implementation scope due to high potential impact (value: ${decisionValue.toFixed(0)})`);
  } else if (Math.abs(decisionValue) < 30) {
    steps.push(`Start with limited pilot implementation due to moderate potential impact (value: ${decisionValue.toFixed(0)})`);
  }
...
function generateWarnings(
  ...
  if (confidenceScore < 0.6) {
    warnings.push(`Low decision confidence (${(confidenceScore * 100).toFixed(0)}%). Consider gathering more information.`);
  }
  
  // Strategic lens mismatch warning
  const lensAlignment = evaluateStrategicLensAlignment(context, strategicLens);
  if (lensAlignment < 0.6) {
    warnings.push(`The ${strategicLens.lens} lens may not be optimal for this context. Consider alternatives.`);
  }
  
  // Timeframe/urgency mismatch warning
  if (context.when.urgency > 8 && context.when.timeframe.toLowerCase().includes('long')) {
    warnings.push(`High urgency (${context.when.urgency}/10) contradicts long-term timeframe.`);
  }
  
  // Resource constraints warning
  if (context.how.constraints && context.how.constraints.length > 2) {
    warnings.push(`Multiple constraints identified (${context.how.constraints.length}). May impact feasibility.`);
  }
  
  // Negative systemic implications warning
  if (macroOffset.systemicImplications && macroOffset.systemicImplications.length > 0) {
    const negativeImplications = macroOffset.systemicImplications.filter(imp => imp.impact < 0);
    if (negativeImplications.length > 0) {
      warnings.push(`${negativeImplications.length} negative systemic implications identified. Consider mitigation strategies.`);
    }
  }
  
  // Flow/Anti-Flow balance warning
  if (flowDynamics.flowRatio > 3 && flowDynamics.disruptionValue > 7) {
    warnings.push(`High disruption value (${flowDynamics.disruptionValue}/10) is being overlooked due to strong flow momentum.`);
  }
  
  // Stakeholder warning
  if (context.who.length < 2) {
    warnings.push(`Limited stakeholder perspective (${context.who.length}). Consider broader involvement.`);
  }
...
function generateInsightSummary(
  ...
  summary += `The ${strategicLens.lens} strategic lens `;
  ...
  if (macroOffset.systemicImplications && macroOffset.systemicImplications.length > 0) {
    const avgImpact = macroOffset.systemicImplications.reduce(
